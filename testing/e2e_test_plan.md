...
- [ ] Click "Reset baseline" again. On the confirmation card the **Reset** button is filled red with white text (matches Delete-button styling). Click "Reset". Toast: "Seen-mail baseline cleared."
...
- [ ] **Chat channel gated.** Open the rule editor. The Google Chat section shows brand-purple text "Google Chat webhooks — upgrade to Pro to use this channel." followed by a plain-style **Upgrade to Pro** button. Clicking the button opens the external upgrade page in a new browser tab (same target as the home-card Upgrade button). The space-selection widget is not rendered.
- [ ] **External integrations channel gated.** Same editor shows brand-purple text "External integrations (Microsoft Teams, Asana, custom MCP, custom webhooks) — upgrade to Pro to use this channel." followed by a plain-style **Upgrade to Pro** button (also opens the external upgrade page). The server-selection widget is not rendered.
- [ ] **AI Help me write the rule text gated.** Below the Rule text field, the editor shows brand-purple text "Upgrade to Pro to use AI assistance for generating rule text." followed by a plain-style **Upgrade to Pro** button (also opens the external upgrade page). The previous "Help me write the rule text (Pro)" button is no longer rendered on Free.
- [ ] **AI Suggest alert content available on Free.** The "Help me write the alert text" button (no "(Pro)" suffix, no upgrade button) is visible in the alert-channel section; clicking it produces a suggestion card from Gemini.
...
- [ ] **AI Help me write the rule text works.** Below the Rule text field a plain-style **Help me write the rule text** button is rendered (no "(Pro)" suffix, no "Upgrade to Pro" text/button). Clicking it opens a card with a multi-line text input where you describe what kinds of emails should match. Clicking Generate sends it to Gemini and produces a suggestion card with **Use this** / **Try again** buttons.
