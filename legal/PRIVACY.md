...
- any message other than the one you have open and explicitly choose to evaluate.
...
|---|---|---|
| **Google Gemini API** (`generativelanguage.googleapis.com`) | Email metadata, body excerpt, rule text, alert format prompt | Each time you click "Evaluate this email" — once per enabled rule, plus once per match to format the alert |
